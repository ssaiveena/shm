function [result] = eval_objs_GW_Recipient_CLD3(Q2,n,rainfall_CA2,PE_CA2)
...
for i=1:12*n  %24 as it is a  fortnightly model
     if i>12*5  
        if deficit2(i-12)~=0 && deficit2(i-12*2)~=0 && deficit2(i-12*3)~=0 && deficit2(i-12*4)~=0 && deficit2(i-12*5)~=0
            Demand_NS(i)  = max(0,Demand_NS(i) - min(2 * deficit2(i-12),250/2));
            live_storage_r2 = min(live_storage_r2+500,6840);
        elseif deficit2(i-12)~=0  
            Demand_NS(i)  = max(0,Demand_NS(i) - min(1*deficit2(i-12),125/2));
        else
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if (mod(i,12) == 7)
    ...
    if (mod(i,12)== 8  || mod(i,12)== 9 || mod(i,12)== 10 ||  mod(i,12)==11||  mod(i,12)==7 )
        ...
        if V2(i)-stor_rel(i)>500
            V2(i) = V2(i) -stor_rel(i);
        else
            V2(i) = min(V2(i),500);
            stor_rel(i) = max(0, V2(i)-500);
        end
    else
         stor_rel(i) = 0;
    end
